  ...
  canceled_{false}
{
  ...
  detection_sub_ = this->create_subscription<vision_msgs::msg::Detection2DArray>(
      "/detector_node/detections", 1, std::bind(&V4L2Camera::detection_callback, this, _1));
  lt_detection_sub_ = this->create_subscription<vision_msgs::msg::BoundingBox2D>(
      "/landing_target_detection", 1, std::bind(&V4L2Camera::lt_detection_callback, this, _1));
  ...
  if(!videoOut.open(encoding, cv::CAP_GSTREAMER, 0, 20, cv::Size(640,480), true)){
    RCLCPP_INFO(get_logger(),"Cannot open video_out");
    return;
  }
  ...
  capture_thread_ = std::thread{
    [this]() -> void {
      while (rclcpp::ok() && !canceled_.load()) {
        ...
        //Convert frame
        frame_ = std::move(img);
        cv_ptr = cv_bridge::toCvCopy(frame_ , frame_->encoding);
        cv::cvtColor(cv_ptr->image, cv_ptr->image, CV_BGR2RGB);

        //TODO: use a timeout for the update of detections, in this way we visualize the same box in the streamied image until
        // a new measurement is found or untile the timeout expires ( ONLY FOR VISUALIZATION PURPOUSE)
        ...
        if(!detections_.empty())
        {
          ...
          {
             cv::Rect rect(det.bbox.center.x-(), det.bbox.center.y,det.bbox.size_x, det.bbox.size_y);
        ...
        if(!lt_detections_.empty())
        {
          for(auto det : lt_detections_)
          {
            //  cv::Rect rect(det.center.x, det.center.y,det.size_x, det.size_y);
            cv::Rect rect(det.center.x-(det.size_x/2), det.center.y-(det.size_y/2),det.size_x, det.size_y);
            cv::rectangle(cv_ptr->image, rect, cv::Scalar(0, 255, 0));
          }
          lt_detections_.clear();
        }

       
        //stream to QgroundControl
        //TODO: resize image to the correct quality for streaming, camera campture can have really high resultion
        if(videoOut.isOpened())
          videoOut.write(cv_ptr->image);
        else
          RCLCPP_ERROR(get_logger(),"VideoOut not opened..");
